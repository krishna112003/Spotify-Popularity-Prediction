...
from Backup.ml_core_old import (
    load_and_clean, merge_datasets, preprocess,
    feature_engineering, select_features, train_all_models,
    predict_new_song, AUDIO_FEATURES,
)
...
if not os.path.exists(css_path):
    # fallback: try sibling style.css
...
# ── Data paths ────────────────────────────────────────────────────────────
TRACKS_PATH = "https://huggingface.co/datasets/krishna112003/Spotify_Music_Analytics_and_Popularity_Prediction/resolve/main/tracks.csv"

ARTISTS_PATH = "https://huggingface.co/datasets/krishna112003/Spotify_Music_Analytics_and_Popularity_Prediction/resolve/main/artists.csv"
...
def build_pipeline(tracks_path, artists_path):
    df_tracks, df_artists = load_and_clean(tracks_path, artists_path)
    df = merge_datasets(df_tracks, df_artists)
    df = preprocess(df)
    df = feature_engineering(df)
    final_features, corr_series = select_features(df)
    return df, final_features, corr_series


@st.cache_resource(show_spinner=False)
def build_models(tracks_path, artists_path):
    df, final_features, _ = build_pipeline(tracks_path, artists_path)
    ...
    with st.sidebar:
        ...
        if data_ok:
            ...
            <div style="padding:.6rem 1rem;background:rgba(239,68,68,0.08);
              ...
              ⚠ Place tracks.csv & artists.csv in /data
            </div>""", unsafe_allow_html=True)

        st.markdown("""
        <div style="position:absolute;bottom:1.5rem;left:1rem;right:1rem;
             font-size:.68rem;color:#334155;text-align:center;line-height:1.6">
          600k+ Spotify Tracks · 1921–2020<br>
          15+ ML Models · Real-time Prediction
        </div>""", unsafe_allow_html=True)
...
def page_overview(df, results, final_features):
    page_hero(
        ...
        subtitle="600,000+ tracks · 7+ ML models · Real-time inference · 1921–2020 dataset",
    )
...
def page_data_explorer(df):
    page_hero(
        ...
        subtitle="Search, filter, sort and download the full Spotify dataset.",
    )
    ...
    with fc2:
        cat_filter = st.multiselect("Category", ["Hit","Mainstream","Emerging","Underground"],
    ...
    try:
        ...
        AgGrid(show_df, gridOptions=go_opts,
               height=480, theme="alpine-dark",
               update_mode=GridUpdateMode.NO_UPDATE,
               data_return_mode=DataReturnMode.FILTERED_AND_SORTED,
               allow_unsafe_jscode=True,
               custom_css={
                   ".ag-root-wrapper": {"border":"1px solid rgba(255,255,255,0.08)!important",
...
def page_analytics(df, corr_series):
    page_hero(
    ...
    with tabs[0]:
        ...
        with lc1:
            ...
            cat_pop = df.groupby("popularity_category")["popularity"].mean().reindex(
        ...
        with lc2:
            ...
            decade_ct = df.groupby("decade").size().reset_index(name="count")
    ...
    with tabs[2]:
        ...
        with rc1:
            ...
            cats_map = {
                "Hit": df[df["popularity_category"]=="Hit"],
                "Mainstream": df[df["popularity_category"]=="Mainstream"],
                "Underground": df[df["popularity_category"]=="Underground"],
            }
            fig = go.Figure()
            radar_colors = {"Hit":"#1DB954","Mainstream":"#3b82f6","Underground":"#7c3aed"}
            radar_fill = {
                "Hit":"rgba(29,185,84,0.15)",
                "Mainstream":"rgba(59,130,246,0.15)",
                "Underground":"rgba(124,58,237,0.15)"
            }
            for cat_name, sub in cats_map.items():
                vals = [sub[f].mean() for f in radar_feats]
                vals += [vals[0]]
                cats_theta = radar_feats + [radar_feats[0]]
                fig.add_trace(go.Scatterpolar(
                    r=vals, theta=cats_theta, name=cat_name,
                    fill="toself",
                    fillcolor=radar_fill[cat_name],
                    line=dict(color=radar_colors[cat_name], width=2),
                ))
        ...
        with rc2:
            section_header("Feature Stats", badge="SUMMARY", icon="📋")
            stat_df = df[radar_feats].describe().T[["mean","std","min","max"]]
            stat_df = stat_df.round(3)
            st.dataframe(
                stat_df.style.background_gradient(
                    cmap="RdYlGn", subset=["mean"]
                ).format("{:.3f}"),
    ...
    with tabs[3]:
        ...
        num_cols_scatter = ["popularity","danceability","energy","loudness",
                            "valence","tempo","acousticness","speechiness",
                            "artist_popularity","track_age"]
        num_cols_scatter = [c for c in num_cols_scatter if c in df.columns]
        with sc1: x_ax = st.selectbox("X Axis", num_cols_scatter, index=1)
        with sc2: y_ax = st.selectbox("Y Axis", num_cols_scatter, index=0)
        with sc3: c_ax = st.selectbox("Color by", ["popularity_category","energy_category",
                                                     "is_modern","explicit"], index=0)

        sample = df.sample(min(5000, len(df)), random_state=42)
        fig = px.scatter(
            sample, x=x_ax, y=y_ax, color=c_ax,
            color_discrete_sequence=COLOR_SEQ,
            opacity=0.55, height=480,
            hover_data={"name": True, "artist_name": True,
                        x_ax: True, y_ax: True},
            trendline=None,
        )
        apply_chart_style(fig, f"{x_ax} vs {y_ax}", 480)
        st.plotly_chart(fig, use_container_width=True)
...
def page_model_lab(results, final_features):
    page_hero(
        eyebrow="🤖 Model Lab",
        title="Training Results &amp; Diagnostics",
        subtitle="Tuned Random Forest · Feature Importance · Confusion Matrix",
    )

    # ── Tuned model KPIs ──────────────────────────────────────────────
    tm = results["tuned_metrics"]
    m1,m2,m3,m4 = st.columns(4)
    with m1: metric_card("Tuned R²",  f"{tm['R2']:.4f}",  color="green",  icon="📐")
    with m2: metric_card("RMSE",      f"{tm['RMSE']:.4f}", color="purple", icon="📉")
    with m3: metric_card("MAE",       f"{tm['MAE']:.4f}",  color="blue",   icon="📏")
    with m4: metric_card("CV R²",     f"{tm['best_cv_r2']:.4f}", color="amber", icon="🔁")
    ...
    # ── Feature Importance ────────────────────────────────────────────
    with tabs[0]:
        section_header("Feature Importance — Tuned Random Forest",
                       badge="TOP DRIVERS", icon="🌲")
        fi = results["feat_imp"].head(20)
        fig = go.Figure(go.Bar(
            x=fi.values[::-1],
            y=fi.index[::-1],
            orientation="h",
        ...
        for i, (feat, imp) in enumerate(fi.head(5).items()):
            ...
            <div style="display:flex;align-items:center;gap:1rem;
              ...
              <span style="color:#475569;font-size:.85rem;width:1.5rem">{i+1}</span>
              <span style="flex:1;font-weight:600;font-size:.88rem">{feat}</span>
              <div style="flex:2;background:rgba(255,255,255,0.06);
...
def page_prediction_studio(results, final_features):
    ...
    with result_col:
        ...
        if predict_btn or "last_pred" in st.session_state:
            ...
            # Feature contribution mini-bars
            fancy_divider()
...
def page_ai_insights(df, results, final_features):
    page_hero(
        ...
        subtitle="15 business insights + 10 Spotify recommendations derived from 600k+ track analysis.",
    )
...
def main():
    ...
    with st.spinner("Loading dataset…"):
        df, final_features, corr_series = build_pipeline(TRACKS_PATH, ARTISTS_PATH)

    # ── Train models ───────────────────────────────────────────────────
    with st.spinner("Training models… (cached after first run)"):
        results, final_features = build_models(TRACKS_PATH, ARTISTS_PATH)
